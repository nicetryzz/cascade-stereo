...
class MVSDataset(Dataset):
    def __init__(self, datapath, listfile, mode, nviews, ndepths=128, interval_scale=1, **kwargs):
    ...
    def read_depth_and_mask(self, filename,depth_min,depth_max):
        # read pfm depth file
        ...
        stage1 = cv2.resize(depth_hr, (w//4, h//4), interpolation=cv2.INTER_NEAREST)
        stage2 = cv2.resize(depth_hr, (w//2, h//2), interpolation=cv2.INTER_NEAREST)
        ...
        mask_hr_ms = {
            "stage1": np.where((stage1 >= depth_min) & (stage1 <= depth_max) , 1., 0.),
            "stage2": np.where((stage2 >= depth_min) & (stage2 <= depth_max) , 1., 0.),
            "stage3": np.where((depth_hr >= depth_min) & (depth_hr <= depth_max) , 1., 0.),
        }
        return depth_hr_ms, mask_hr_ms
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            if i == 0:  # reference view
                depth_ms, mask_ms = self.read_depth_and_mask(depth_filename_hr, depth_min, depth_max)
                #get depth values
